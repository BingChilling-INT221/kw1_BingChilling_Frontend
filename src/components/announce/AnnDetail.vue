  ...
  <div class="mx-[10%] w-[80%] pt-10">
    ...
    <div v-if="loading" class="flex justify-center min-w-full min-h-full text-center bg-slate-400">
      <div class="absolute mt-2 mr-2">
        <svg class="w-20 h-20 bg-transparent border-2 border-transparent border-opacity-50 rounded-full animate-spin" style="border-right-color: white; border-top-color: white" viewBox="0 0 24 24"></svg>
      </div>
    </div>
      ...
      <div class="w-full px-8 pt-4 pb-4 rounded-lg">
        <div class="flex justify-between font-bold">
          <p class="w-4/5 text-xl break-words ann-title">
            Title: {{ queryAnnounce.announcementTitle }}
          </p>
          <div v-show="isAdminPath" class="flex justify-end">
            <div :class="[queryAnnounce.announcementDisplay === 'Y' ? 'bg-green-500' : 'bg-red-500']" class="flex justify-center w-24 h-10 p-2 py-3 bg-green-500 rounded-lg ann-display sm:w-28 sm:h-12">
        ...
        <div class="py-5 ann-category">
          <a class="bg-[#628FB8] px-5 text-sm rounded-lg py-1" href="#">{{ queryAnnounce.announcementCategory }}</a>
        </div>
        ...
        <div class="flex justify-end">
          <button v-show="isAdminPath" class="px-2 py-1 mt-2 ml-6 border-2 rounded-lg ann-button hover:bg-gray-300" @click="$router.push({ name: 'editannouncement', params: { id: queryAnnounce.id } })">edit</button>
        </div>
      </div>
    </div>
  </div>

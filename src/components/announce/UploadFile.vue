...
const selectFiles = () => {
  ...
  const fileInputRef =
    fileInput.value instanceof HTMLInputElement ? fileInput.value : null;
  if (fileInputRef) {
    fileInputRef.click();
  }
};
const uploadDefault = (event, index, action) => {
  ...
  const file2InputRef =
    fileInputRefs.value instanceof HTMLInputElement
      ? fileInputRefs.value
      : null;
  if (file2InputRef) {
...
const previewFile = (file, index, action) => {
  ...
  reader.onload = () => {
    ...
    if (action === "reset") {
      ...
      if (filesPreview.value[index].fileName === obj.previewName) {
        update(obj.previewType, obj.previewUrl, obj.previewName, index);
      }
      else  if (
        filesPreview.value.every((item,indexloop) => {
          // console.log("item : ", item);
          return item.fileName !== obj.previewName;
        }) ||
        oldFiles.value.some((item) => item === obj.previewName)
      ) {
        update(obj.previewType, obj.previewUrl, obj.previewName, index);
      ...
      if (
        filesPreview.value.every((item) => item.fileName !== obj.previewName)
      ) {
        add(obj.previewType, obj.previewUrl, obj.previewName);
      } else {
        alert("File already exists");
        return;
      }
      ...
      if (
        props.preview.every((item) => item.fileName !== obj.previewName) ||
        oldFiles.value.some((item) => item === obj.previewName)
      ) {
...
const add = (previewType, previewUrl, previewName) => {
  // console.log('add');
...
const update = (previewType, previewUrl, previewName, index) => {
  // console.log('index : ', index);
  if (props.preview[index] !== undefined) {
...
const removeImg = (index) => {
  ...
  if (
    props.preview.some(
      (item) => item.fileName === filesPreview.value[index].fileName
    )
  ) {
    oldFiles.value.push(filesPreview.value[index].fileName);
  } else {
...
const handleDrop = (event, index, action) => {
  ...
  else if (files.length > 1 && action === "reset") {
    // alert("Max file is 1");
    files = [files[0]];
  }
  // console.log("files : ", files);
    ...
    <div v-for="(file, k) in filesPreview" :key="k" class="flex cursor-pointer">
      ...
      <div v-if="file" class="m-5">
        ...
        <preview-file-c
          ...
          :previewType="file.fileType"
          :previewUrl="file.fileUrl"
          v-if="file"
          @click="selectFile($event,k)"

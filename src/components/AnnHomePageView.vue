        ...
        <div v-else class="flex pt-16">
            <div class="lg:basis-4/6 flex flex-col ">
                <div class="flex justify-between ">
                    <p class="text-2xl font-semibold pl-5   md:max-h-16 md:min-h-16">Announcement</p>
                    <div v-if="!checkAdmin()" class="flex ">
                        <button class="px-2 py-1   rounded-md ann-button "
                                @click="fetches()">
                            {{ isOpen ? 'Closed announments' : 'Open announments' }}
                        </button>
                        <div :class="isOpen?'bg-green-400':'bg-red-400'" class="w-3 h-3  rounded-full m-auto"></div>
                    </div>
                    <button v-else
                ...
                <p class="py-1 text-xl font-normal ">Date/Time shown in Timezone: <span class="font-normal">{{
                    ...
                    <div class="flex font-normal justify-between">
                        ...
                        <select
                                ...
                                class="text-black shadow-md shadow-slate-300 ann-category-filter pl-10 " required>

  ...
  <div class="p-2">
    <div class="border-2 m-auto p-4 rounded-lg">
      ...
      <form action="">
        ...
        <input type="text" class="border-2 rounded-md w-full py-1 ann-title" v-model="announcementTitle" />
        <p class="text-white font-bold py-2">Category</p>
        <select class="w-3/12 shadow-md shadow-slate-300 ann-category" v-model="categoryName">
          <option :value="data.categoryName" v-for="(data,index) in category" :key="data.id">{{ data.categoryName }}</option>
        </select>
        ...
        <p class="text-white">
        {{publishDatePlueTime ?? 'null'}}
        {{ closeDatePlueTime ?? 'null'}}
        {{ announcementDisplay }}
        {{ categoryName }}
      </p>
      </form>
